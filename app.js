...
const { getSupabase, getSanitizedUrl, withTimeout } = require('./db');
const storage = require('./storage');

console.log('[BOOT] app.js loaded, region=', process.env.VERCEL_REGION || 'local');
...
// ============================================================
// PING — pure function check, geen Supabase
// ============================================================
...
app.get('/api/health', async (req, res) => {
  const result = {
    env: {
      SUPABASE_URL: getSanitizedUrl() ? `${getSanitizedUrl().slice(0, 35)}...` : 'MISSING',
  ...
  try {
    const t0 = Date.now();
    const r = await withTimeout(getSupabase().from('members').select('id', { count: 'exact', head: true }), 6000, 'health');
...
app.get('/api/rides', async (req, res) => {
  const { data, error } = await getSupabase().from('rides')
...
app.post('/api/rides', upload.single('photo'), async (req, res) => {
  try {
    ...
    const { data: ride, error: insErr } = await getSupabase().from('rides').insert({
    ...
    if (linkErr) {
      await getSupabase().from('rides').delete().eq('id', ride.id);
      throw linkErr;
    }

    const { data: full } = await getSupabase().from('rides').select('*, ride_members ( members ( id, name ) )')
...
app.delete('/api/rides/:id', async (req, res) => {
  try {
    ...
    const { data: row } = await getSupabase().from('rides').select('photo').eq('id', id).single();
...
app.get('/api/members/:id/rides', async (req, res) => {
  try {
    const id = Number(req.params.id);
    const { data, error } = await getSupabase().from('ride_members')
...
app.get('/api/sponsors', async (req, res) => {
  const { data, error } = await getSupabase().from('sponsors').select('*')
...
app.post('/api/sponsors', upload.single('logo'), async (req, res) => {
  try {
    ...
    const { data, error } = await getSupabase().from('sponsors').insert({
...
app.delete('/api/sponsors/:id', async (req, res) => {
  try {
    const id = Number(req.params.id);
    const { data: row } = await getSupabase().from('sponsors').select('logo').eq('id', id).single();
    if (!row) return res.status(404).json({ error: 'Niet gevonden' });
...
app.post('/api/events', async (req, res) => {
  try {
    ...
    const { data, error } = await getSupabase().from('events').insert({
...
app.delete('/api/events/:id', async (req, res) => {
  try {
    const id = Number(req.params.id);
    const { error, count } = await getSupabase().from('events').delete({ count: 'exact' }).eq('id', id);
    if (error) throw error;
...
app.get('/api/stats', async (req, res) => {
  const { data: raw, error } = await getSupabase().from('rides').select('*, ride_members ( members ( id, name ) )');
  if (error) return fail(res, error);
